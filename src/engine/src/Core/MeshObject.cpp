...
// TODO consider creating a group with MeshNode - (Mesh) TransformComponent
// Need to consider how to better organize this data in the registry...
...
void Mesh::ForEachNode(std::function<void(MeshNode*)> func) const {
    MeshComponent& rootComponent = _scene->GetComponents<MeshComponent>(_entity);
    
    for(auto& rootNode : rootComponent._meshNodes) {
        func(&rootNode);
        
        for(MeshNode& childNode : rootNode._childNode) {
            func(&childNode);
        }
    }
}
