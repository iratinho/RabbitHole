...
void VKBuffer::Initialize() {
    ...
    if(_type & EBufferType::BT_HOST) {
        ...
        std::tie(_stagingBuffer, _stagingBufferMemory) = makeBuffer(usage, (VkMemoryPropertyFlagBits) (VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT));
    }
    ...
    if(_type & EBufferType::BT_LOCAL) {
        ...
        std::tie(_localBuffer, _localBufferMemory) = makeBuffer(usage, (VkMemoryPropertyFlagBits) VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    }
