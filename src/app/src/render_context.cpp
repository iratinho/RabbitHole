...
bool RenderContext::CreateVulkanInstance()
{
    ...
    const VkResult instance_enumeration_result = vkEnumerateInstanceLayerProperties(
    ...
    const VkResult instance_extensions_result = vkEnumerateInstanceExtensionProperties(
...
bool RenderContext::PickSuitableDevice()
{
    ...
    const VkResult physical_device_result = vkEnumeratePhysicalDevices(instance_, &device_count,
    ...
    for (const VkPhysicalDevice& physical_device_handle : physical_device_handles)
    {
        ...
        if (device_properties.apiVersion >= loader_version_)
        {
            ...
            vkGetPhysicalDeviceQueueFamilyProperties(physical_device_handle, &queue_family_properties_count,
                                                     queue_family_properties.data());

            uint32_t device_extensions_properties_count = 0;
            vkEnumerateDeviceExtensionProperties(physical_device_handle, nullptr, &device_extensions_properties_count,
                                                 nullptr);
            ...
            for (const auto queue_family_property : queue_family_properties)
            {
                ...
                vkGetPhysicalDeviceSurfaceSupportKHR(physical_device_handle, queue_family_index, surface_,
                                                     &allows_presentation);
...
bool RenderContext::CreateLogicalDevice()
{
    ...
    const VkResult create_device_result = vkCreateDevice(device_info_.physical_device, &device_create_info, nullptr,
                                                         &logical_device_);
...
bool RenderContext::CreateSwapChain()
{
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(device_info_.physical_device, surface_, &surface_formats_count,
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(device_info_.physical_device, surface_, &present_mode_count,
                                              present_modes.data());
...
void RenderContext::DestroyImageView(VkImageView image_view) {
    vkDestroyImageView(GetLogicalDeviceHandle(), image_view, nullptr);
}
